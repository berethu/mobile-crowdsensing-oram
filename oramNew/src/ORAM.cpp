...
ORAM::ORAM(BlockStore *store, size_t depth, size_t blockSize, bytes<Key> key, std::string stashbin, std::string posbin)
: store(store), depth(depth), blockSize(blockSize), key(key), stashbin(stashbin), posbin(posbin), position(new int[GetBlockCount()]), rd(), mt(rd()), dis(0, GetBucketCount()/2)
{
	...
	bool stashExists = StashHelper::Load(stashbin, stash, blockSize);
	
	bool positionExists = PositionHelper::Load(posbin, position, GetBlockCount());
...
ORAM::~ORAM()
{
	PositionHelper::Save(posbin, position, GetBlockCount());
	StashHelper::Save(stashbin, stash, blockSize);
}
...
block ORAM::SerialiseBucket(Bucket bucket)
{
	...
	for (int z = 0; z < Z; z++) {
		...
		//assert(b.data.size() == blockSize); Emmanuel

		// Write block data
		buffer.insert(buffer.end(), b.data.begin(), b.data.end());
	}

	//assert(buffer.size() == Z*(sizeof (int32_t) + blockSize)); Emmanuel
...
{
	//assert(b.size() == blockSize); Emmanuel
...
{
	//assert(blockSize == b.size());  Emmanuel
